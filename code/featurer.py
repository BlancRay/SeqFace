...
class Featurer():
    def __init__(self, deploy_prototxt, model_file, mean_file=None, ratio_file=None, label_file=None, device_id=-1,
                 score_thred=0.0, top_k=1, ratio=-1.0, scale=1.1, resize_height=256, resize_width=256, raw_scale=255.0,
                 input_scale=1.0, gray=False, oversample=False, feature_layer_names=None):
        self.ratio = ratio
        self.scale = scale
        self.gray = gray
    ...
    def test(self, image_file, box=None, feature_layer_names=None, score_thred=0.0, top_k=1, ratio=None, scale=None,
             oversample=None, timeout=2):
        if ratio is None:
            ratio = self.ratio
        if scale is None:
            scale = self.scale
